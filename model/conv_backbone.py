...
class Block3D(nn.Module):
    ...
    def __init__(self, dim, drop_path=0., inflate_len=3, layer_scale_init_value=1e-6):
        super().__init__()
        self.dwconv = nn.Conv3d(dim, dim, kernel_size=(inflate_len, 7, 7), padding=(inflate_len // 2, 3, 3),
        ...
        self.grad = []
        self.features = []
        # self.dwconv.register_forward_hook(self.save_features_hook) # 注册hook
...
class ConvNeXt3D(nn.Module):
    ...
    def __init__(self, in_chans=3, num_classes=1000,
        ...
        self.norm = nn.LayerNorm(dims[-1], eps=1e-6) # final norm layer
        # self.new_norm = nn.LayerNorm(960, eps=1e-6) # new final norm layer
        # self.dense1 = nn.Sequential(
        #     nn.Conv3d(in_channels=96, out_channels=64, kernel_size=(1, 8, 8), stride=(1, 8, 8)),
        #     nn.GELU()
        # )
        # self.dense2 = nn.Sequential(
        #     nn.Conv3d(in_channels=192, out_channels=64, kernel_size=(1, 4, 4), stride=(1, 4, 4)),
        #     nn.GELU()
        # )
        # self.dense3 = nn.Sequential(
        #     nn.Conv3d(in_channels=384, out_channels=64, kernel_size=(1, 2, 2), stride=(1, 2, 2)),
        #     nn.GELU()
        # )
        self.apply(self._init_weights)
    ...
    def forward_features(self, x, return_spatial=False, multi=False, layer=-1):
        ...
        for i in range(4):
            x = self.downsample_layers[i](x)
            x = self.stages[i](x)
            # if i == 0: feats = self.dense1(x)
            # elif i == 1: feats = torch.cat((feats, self.dense2(x)), dim=1)
            # elif i == 2: feats = torch.cat((feats, self.dense3(x)), dim=1)
            # elif i == 3: feats = torch.cat((feats, x), dim=1)
            if multi:
                xs.append(x)
        if return_spatial:
            if multi:
